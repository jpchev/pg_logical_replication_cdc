...
pattern_col_data = re.compile(r"col_data=(None|'[^']*')", re.M)
...
def create_insert_message_json(splitted_records, insertdata):
    ...
    col_data = get_col_data(insertdata)
...
def create_update_message_json(splitted_records, updatedata):
    ...
    col_data = get_col_data(updatedata)
...
def get_col_data(data):
    def map_null(el):
        return None if el == 'None' else el.strip("\"'")
    return list(map(map_null, re.findall(pattern_col_data, data)))
